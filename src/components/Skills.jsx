...
export default function Skills() {
    ...
    return (
        <div className='bg-secondary lg:h-screen w-full py-20'>
            <h2 className='text-center text-3xl font-semibold my-4 md:text-4xl'>My <span className='text-primary'>Expertise</span></h2>
            <div className='my-7 grid grid-cols-2 mobilelarge:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 place-items-center gap-y-8'>
                {
                    skills.map((item, index) => (
                        <img key={index} src={item.image} alt="Skill" className="w-24 hover:scale-110 cursor-not-allowed transition-all duration-200" />
                    ))
                }
            </div>
        </div>
